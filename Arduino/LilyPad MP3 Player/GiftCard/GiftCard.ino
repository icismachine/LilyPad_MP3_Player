...
void SetupGPIOs()
{
  ...
  Serial.println("make triggers");
  for (x = 0; x < TRIGGERS; x++)
  {
    pinMode(trigger[x],INPUT);
    digitalWrite(trigger[x],HIGH);
    Serial.print("Create Trigger on: ");
...
void ScanForAudioFiles()
{
  ...
  // starting with the characters '1' to '5' upto AUDIO_FILES:
  ...
  while (file.openNext(sd.vwd(),O_READ))
  {
    ...
    //ICIS : move to strstr compare
    if (sscanf(tempfilename, "%d",&index) && (index > 0))
    {
   ...
   //check trigger LOW
    ...
    if (digitalRead(trigger[0]) == LOW)
    {
      ...
      else // We do have a filename for this trigger!
      {
        ...
        if (interrupt && MP3player.isPlaying() && ((CurrentIndex != last_t) || interruptself))
        {
          if (debugging)
        ...
        if(debugging)
        {
          ...
          Serial.println(filename[CurrentIndex]);
        }
      }
    }
